...
AGUIWidget::AGUIWidget() : mPosition(0, 0), mSize(0, 0), mAbsolutePosition(0, 0) {
    mParent = nullptr;
...
void AGUIWidget::destroyChild(AGUIWidget *&widget) {
    if (!widget) {
        LOG_F(WARNING, "widget is nullptr");
    }
    else {
        ...
        if (it != mChildren.end()) {
            ...
            LOG_F(WARNING, "Failed to destroy GUIWidget. The widget wasn't registered at parent as child");
        }
    }
}
