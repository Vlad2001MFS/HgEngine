...
AECSComponent* SceneSystem::mCreateComponent(HEntity& handle, uint64_t typeHash, const std::string& typeName) {
    ...
    if (factory == mComponentTypes.end()) {
        LOG_F(FATAL, "Failed to create component of not registered type '{}'", typeName.data());
        return nullptr;
    }
    if (handle && handle.value < mEntities.size()) {
        ...
        LOG_F(FATAL, "Failed to create component '{}' for entity '{}'", factory->second.first.data(), handle.value);
        return nullptr;
    }
}
